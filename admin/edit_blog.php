...
$sql2="SELECT * FROM blog LEFT JOIN categories ON blog.categories=categories.cat_id
...
<select class="form-control" name="category" required> 
        ...
        <?= ($result['categories']==$cats['cat_id'])?"Selected":'';?>>
        <?= $cats['cat_name'] ?>
        </option>
        <?php } ?>
</select>
...
if(isset($_POST['edit_blog'])){
    ...
    if(!empty($filename)){
        if(in_array($image_ext,$allow_type)){
            if($size <= 2000000){
                ...
                $sql3="UPDATE blog SET blog_title='$title',blog_body='$body',blog_image='$filename',categories='$category',
                author_id='$author_id' WHERE blog_id='$blogID'";
             ...
             $msg=['image size should not be greater than 2 mb','alert-danger'];
             $_SESSION['msg']=$msg;
             header("location:index.php");
            }
         }
    ...
    {
        $sql3="UPDATE blog SET blog_title='$title',blog_body='$body',categories='$category',
        author_id='$author_id' WHERE blog_id='$blogID'";
        $query3=mysqli_query($config,$sql3);
        if($query3){
         $msg=['Post updated successfully','alert-success'];
